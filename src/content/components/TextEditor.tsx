...
const TextEditor: React.FC<TextEditorProps> = ({
  ...
  // Modern highlight color options
  const highlightColors = [
    "#FFF176", // Light Yellow
    "#FFB74D", // Light Orange
    "#4FC3F7", // Light Blue
  ...
  const formatSelection = (
    ...
    // Show color picker if highlighting
    if (formatType === "highlight" && existingFormatIndex < 0) {
  ...
  const updateHighlightColor = (color: string) => {
    ...
    // If we have a selection with highlight formatting, update it
    if (selectedRange) {
      const highlightFormatIndex = richContent.formatting.findIndex(
        (fmt) =>
          fmt.type === "highlight" &&
          fmt.start === selectedRange.start &&
          fmt.end === selectedRange.end
      ...
      if (highlightFormatIndex >= 0) {
        const newFormattingArray = [...richContent.formatting];
        newFormattingArray[highlightFormatIndex] = {
          ...newFormattingArray[highlightFormatIndex],
          color,
        };
        ...
        segment.formats.forEach((format) => {
          switch (format.type) {
            ...
            case "underline":
              ...
              break;
            case "highlight":
              styledContent = `<span class="highlight" style="background-color: ${
                format.color || "#FFFF00"
              }; padding: 0 2px; border-radius: 2px;">${styledContent}</span>`;
              break;
          }
        });
        ...
        {!isSingleLine && (
          <>
            <div className="flex flex-wrap gap-1 border-t pt-2 mt-2 pb-1">
              ...
              {isColorPickerOpen && (
                <div
                  className="absolute z-10 mt-8 bg-white border border-gray-200 rounded shadow-lg p-2"
                ...
                >
                  ...
                  <button
                    className="w-full text-xs text-gray-600 mt-2 hover:text-gray-800"
